...
//		debug($data);
		...
		if(isset($authUrl)) //user is not logged in, show login button
		{
			...
			//set session and load it into the view
			 ...
			 $gdata['given_name'] = $user['given_name'];
			 $gdata['family_name'] = $user['family_name'];
			 $gdata['locale'] = $user['locale'];
			 $gdata['name'] = $user['name'];
			 $this->session->set_userdata("gmail_data",$gdata);
			 $data["google_data"] = '<div class=" col-md-9 col-sm-6 col-xs-12 google">
	...
	//	debug($this->input->post());die;
		$check = $this->session->userdata("signup_user_type"); 
        ...
        if($check == 'organisation') {
            ...
            if($insert)
			{
				...
				$onsite_data = array('login_userid' => $arr2["user_id"],'email' =>$arr["email"]);
				...
				$this->session->unset_userdata('gmail_data');//unset google data
            ...
            if($insert)
			{
				$arr2["user_id"] = $this->db->insert_id();
				//$this -> login_model -> add_meta_user($arr2);
				//$this->session->set_userdata("login_userid",$arr2["user_id"]);
				$onsite_data = array('login_userid' => $arr2["user_id"],'email' =>$arr["email"]);
				$this->session->set_userdata("signup_userdata",$onsite_data);
		...
		public function setUserType()
		{
			...
			if($type <> ""){
				$this->session->set_userdata("signup_user_type",$type);
				echo $this->session->userdata("signup_user_type");
				die;
			}
		}
		...
		public function send_email()
		{
            ...
            $user_type = $this->session->userdata('signup_user_type');
