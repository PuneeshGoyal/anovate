...
if (!$o) {
	...
	echo '<th colspan="7" class="cal_top" style="text-align:center;">
	<input type="hidden" value="" id="temp_value">
	<a href="',$PHP_SELF,'?cause_id='.$_GET["cause_id"].'&do='.$_GET["do"].'&o=',$o,'&w=',$w,'&c=',$c,'&m=',$prev["month"]["m"],'&a=1&y=',$prev["month"]["y"],'">&lt;</a> ',date('F', mktime(0,0,0,$calmonth,1,$calyear)),'&nbsp;',date('Y', mktime(0,0,0,$calmonth,1,$calyear)),' <a href="',$PHP_SELF,'?cause_id='.$_GET["cause_id"].'&do='.$_GET["do"].'&o=',$o,'&w=',$w,'&c=',$c,'&m=',$next["month"]["m"],'&a=1&y=',$next["month"]["y"],'">&gt;</a></th></tr><tr>';
	for ( $x = 0; $x < 7; $x++ )
			//echo $x;
	...
	for($d = 1; $d <= $totaldays; $d++ ){
			...
			if (($d == date(j)) && ($calmonth == date(m)) && ($calyear == date(Y))) {
				...
				if($time_passes==1){$tim='';}else{$tim='partial-booked';}
