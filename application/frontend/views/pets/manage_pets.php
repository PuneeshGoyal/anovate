...
<div class="container"> 
  ...
  <div class="row accnt_parah">
    <div class="col-md-9 detail">
      <p><?php echo !empty($data_user['name']) ? $data_user['name'] : "";?></p>
    </div>
    ...
    <div id="content">
      <ul id="tabs" class="nav nav-tabs acc_custom_nav_tabs" data-tabs="tabs">
        ...
        <li><a href="<?php echo base_url();?>accounts/manage_causes" >Manage Your Causes</a></li>
         <li class="active"><a href="<?php echo base_url();?>pets/manage_pets" >Manage Your Pet Listing</a></li>
            ...
            <table class="button_styles table manage_table appointment_table table-condensed table-striped table-bordered table-hover no-margin">
              <thead>
                <tr >
                  ...
                  <td width="20%">
                  <form data-toggle="tooltip" data-placement="right" title="Filter records" action="<?php echo  base_url().$this->router->class;?>/manage_pets/" method="get" id="search" name="filter" onchange="this.form.submit()">
              ...
              <tbody>
                ...
                <?php 
				 ...
				 foreach($resultdata as $key=>$val){
				 	...
				 	($val["id"] != "") ? (list($link1, $is_fundraise) = array(base_url().$this->router->class.'/view_appointment/'.$val['slug'], '<a rel="tooltip" data-placement="right" title="Click here to see more information" class="popup_button" style="text-decoration:none;" data-toggle="modal"  href='.base_url().$this->router->class.'/view_appointment/'.$val['slug'].' data-target="#view_appointment_modal" oncontextmenu="return false">View appointments </a>')) :(list($link1, $is_fundraise) = array(NULL, NULL));
                  ...
                  <td><!-- <input type="checkbox" class="checkbox"  name="chk[]" value="<?php echo $val['id'];?>"/> -->
                    ...
                    <div class="input-group-btn">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right" role="menu">
                          ...
                          <li><?php
						   if($val['steps_completed'] == 0 && $val['status']==1){?>
							 <button type="button" value="Edit" class="btn btn-primary" onclick="window.location.href = '<?php echo base_url().$this->router->class;?>/edit_step2/<?php echo $val['id'];?>'"/><span class="glyphicon glyphicon-edit"></span>Edit</button> 
						   <?php } else if($val['steps_completed'] == 1 && $val['status']==1){?>
                            <button type="button" value="Edit" class="btn btn-primary" onclick="window.location.href = '<?php echo base_url().$this->router->class;?>/edit_pets/<?php echo $val['slug'];?>'"/><span class="glyphicon glyphicon-edit"></span>Edit</button>
                            <?php }else {?>
          ...
          <div class="row">
            ...
            <div class="col-md-7 list_donor"> 
              <!-- <input data-toggle="modal" data-target="#myModal11" type="button" value="List of Donors" class="add list"/>-->
               <button data-toggle="tooltip" data-placement="left" title="Start a pet listing!" class="btn btn-primary" type="button" onclick="window.location.href = '<?php echo base_url();?>pets/'">Add new</button>
...
<div class="modal fade" id="view_appointment_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<script src="<?php echo base_url();?>js/manage_pets.js"></script> 
