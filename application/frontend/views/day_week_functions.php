...
function showDay($dy,$dm,$da,$caption="") { 
	...
	if(time() > $time1){$disabled_days = 'disabled_days'; $full='';}
	else{$disabled_days=''; $full='booked';}
	...
	/*else if(in_array($selected_dat, $selectedDate) && getCalviewType($selected_dat) != "month"){
		...
		//week_delete
		if(in_array($time2,$dates)){ $class = " cell vik partial-booked "; }else { $class= "cell vik ".$disabled_days.""; }
	}
	...
	while ($i < $max) {
		...
		/*else if(in_array($selected_dat, $selectedDate) && getCalviewType($selected_dat) != "month"){
			...
			//week_delete
			if(in_array($time,$dates)){ $class = "cell vik partial-booked "; }
			else { $class= "cell vik ".$disabled_days.""; }
		}
