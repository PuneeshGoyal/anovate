...
<div class="container cause_pad">
  <div class="row stories_container">
    <form class="form-horizontal story_hopes_new" id="search" role="form" method="get" action="<?php echo base_url();?>hope/">
      ...
      <div class="form-group col-md-3 col-sm-6 col-xs-12 new_country">
        ...
        <div class="col-sm-8 col-md-8 col-xs-12" id="country">
          <select id="selectbasic" name="country" class="multiselect" multiple="multiple">
            ...
            <option value="singapore" <?php if($country  == $_GET['country']){echo 'selected=selected';}?>>Singapore</option>
          </select>
        </div>
      </div>
      ...
      <div class="form-group col-md-2 col-sm-6 col-xs-12 type"> 
        ...
        <label class="col-md-3 col-sm-4 control-label custm_labl"></label>
        <button type="button" class="btn btn-primary search_btn" onclick="submit_request();">Search</button>
      </div>
    </form>
  </div>
</div>
...
<div class="container caus_fst"> 
  <!-- Ist row-->
   ...
   foreach($causedata as $key=>$val){
		...
		$type = implode(', ',$temp);
		if($val["status"]=="3"){$close='<div class="arrow-right"></div>';}else{$close='';}
	?>
   <!-- new div starts here -->
...
<script type="text/javascript">
	...
	function submit_request()
	{
	 ...
	 var url =  BASEURL+"hope/?do=search&country="+country+"&category="+category+"&type="+type;
